...
- Testing deploys to the **same** self-hosted runner as production (`[self-hosted, dinner-planner]`), as a second `dinner-planner-testing` container on port **3002** (prod stays on 3000; 3001 was already taken by an unrelated `anythingllm` container on that host — discovered live during the first promotion run, port bumped to 3002 both in the host's `compose.yml` and the NPM proxy target), separate data volume, separate `TESTING_JWT_SECRET`/`TESTING_ADMIN_PASSWORD`. Fronted by Nginx Proxy Manager at `dinner-test.duckpuppy.net` → `192.168.0.95:3002`. No CORS/cookie-domain changes were needed for the new subdomain — the app already serves SPA+API same-origin and the refresh-cookie logic already binds to whatever host is used (see cookie-domain gotcha above).
