...
describe('GroceryPage', () => {
  ...
  describe('scale selector', () => {
    it('shows Scale: label and 1×, 2×, 4× buttons after loading', async () => {
      vi.mocked(menus.getGroceries).mockResolvedValue({
        groceries: [
          {
            name: 'Flour',
      ...
      expect(screen.getByRole('button', { name: /2×/i })).toBeTruthy();
      expect(screen.getByRole('button', { name: /4×/i })).toBeTruthy();
    });

    it('doubles displayed quantity when 2× scale is selected', async () => {
      vi.mocked(menus.getGroceries).mockResolvedValue({
        groceries: [
          {
            name: 'Flour',
      ...
      // Now 1000 should be shown
      expect(screen.getByText(/1000/)).toBeTruthy();
    });

    it('quadruples displayed quantity when 4× scale is selected', async () => {
      vi.mocked(menus.getGroceries).mockResolvedValue({
        groceries: [
          {
            name: 'Eggs',
    ...
    it('resets to 1× and restores original quantity', async () => {
      vi.mocked(menus.getGroceries).mockResolvedValue({
        groceries: [
          {
            name: 'Butter',
    ...
    it('1× button has aria-pressed=true by default', async () => {
      vi.mocked(menus.getGroceries).mockResolvedValue(baseGroceriesResponse);
      render(<GroceryPage />, { wrapper });
